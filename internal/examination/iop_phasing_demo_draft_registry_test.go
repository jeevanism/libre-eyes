...
func TestIOPPhasingDemoDraftRegistryValidate(t *testing.T) {
	...
	if err := (IOPPhasingDemoDraftRegistry{}).Validate(context.Background(), iopPhasingDemoEventType, episodes.DraftIntentCreate, 1, raw); err != nil {
		t.Fatal(err)
	}
	payload["rightEye"].(map[string]any)["readings"] = []map[string]any{{"value": 101, "measurementTime": "09:00"}}
	raw, _ = json.Marshal(payload)
	if err := (IOPPhasingDemoDraftRegistry{}).Validate(context.Background(), iopPhasingDemoEventType, episodes.DraftIntentCreate, 1, raw); err != episodes.ErrInvalidRequest {
		t.Fatalf("got %v", err)
	}
}

func TestIOPPhasingDemoDraftRegistryProductionGuard(t *testing.T) {
	if _, err := NewIOPPhasingDemoDraftRegistry("production"); err == nil {
