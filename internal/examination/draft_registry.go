...
func NewDraftRegistry(ctx context.Context, pool *pgxpool.Pool, environment string) (*DraftRegistry, error) {
	...
	catpromDemo, err := NewCatpromDemoDraftRegistry(environment)
	if err != nil {
		return nil, err
	}
	dnaExtractionDemo, err := NewDNAExtractionDemoDraftRegistry(environment)
	if err != nil {
		return nil, err
	}
	dnaSampleDemo, err := NewDNASampleDemoDraftRegistry(environment)
	if err != nil {
		return nil, err
	}
	...
	therapyIntentDemo, err := NewTherapyIntentDemoDraftRegistry(environment)
	if err != nil {
		return nil, err
	}
	pgdPSDGuidanceDemo, err := NewPGDPSDGuidanceDemoDraftRegistry(environment)
	if err != nil {
		return nil, err
	}
	geneticResultDemo, err := NewGeneticResultDemoDraftRegistry(environment)
	if err != nil {
		return nil, err
	}
...
func (r *DraftRegistry) Validate(ctx context.Context, eventTypeCode string, intent episodes.DraftIntent, schemaVersion int64, payload json.RawMessage) error {
	...
	case catpromDemoEventType:
		return r.catpromDemo.Validate(ctx, eventTypeCode, intent, schemaVersion, payload)
	case dnaExtractionDemoEventType:
	...
	case therapyIntentDemoEventType:
		return r.therapyIntentDemo.Validate(ctx, eventTypeCode, intent, schemaVersion, payload)
	case pgdPSDGuidanceDemoEventType:
