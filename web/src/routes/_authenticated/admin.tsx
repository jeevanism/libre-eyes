...
function AdminPage() {
  ...
  return <div className="admin-workspace">
    ...
    <div className="admin-grid">
      ...
      <section className="admin-panel"><h2><Settings size={18} /> Operational configuration</h2>{settings.map((setting, index) => <div key={setting.key}>{(index === 0 || settingGroup(settings[index - 1].key) !== settingGroup(setting.key)) && <h3 className="admin-setting-group">{settingGroup(setting.key)}</h3>}<div className="admin-row"><span>{settingLabel(setting.key)}</span><span>{setting.key === 'default_site' || setting.key === 'default_firm' ? <select aria-label={setting.key} value={setting.value} onChange={event => settingMutation.mutate({ key: setting.key, value: event.target.value, version: setting.version })}>{(setting.key === 'default_site' ? contexts?.sites : contexts?.firms)?.filter(item => item.active).map(item => <option key={item.id} value={item.name}>{item.name}</option>)}</select> : <input aria-label={setting.key} defaultValue={setting.value} onBlur={event => { const value = event.currentTarget.value; if (value !== setting.value) settingMutation.mutate({ key: setting.key, value, version: setting.version }) }} />}</span></div></div>)}</section>
