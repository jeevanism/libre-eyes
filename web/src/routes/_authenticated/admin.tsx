...
function AdminPage() {
  ...
  return <div className="admin-workspace">
    ...
    <div className="admin-grid">
      <section className="admin-panel"><h2><Users size={18} /> Users</h2>{users.map(user => <div className="admin-row" key={user.id}><strong>{user.displayName}</strong><span>{user.role.replaceAll('_', ' ')}</span><span>{user.active ? 'Active' : 'Inactive'}</span></div>)}</section>
      ...
      <section className="admin-panel"><h2><Settings size={18} /> Demo settings</h2>{settings.map(setting => <div className="admin-row" key={setting.key}><span>{setting.key.replaceAll('_', ' ')}</span><strong>{setting.value}</strong></div>)}</section>
      <section className="admin-panel"><h2><History size={18} /> Audit summary</h2>{audit.length === 0 ? <p className="muted">No administration events yet.</p> : audit.map((event, index) => <div className="admin-row" key={`${event.command}-${index}`}><span>{event.command}</span><span>{event.targetType}</span><span>{event.outcome}</span></div>)}</section>
    </div>
  </div>
}
