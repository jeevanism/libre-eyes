...
export function VisualFieldsDraftDemo({ csrfToken, episodeId }: { csrfToken: string; episodeId: string }) {
  ...
  function submit(event: FormEvent) { event.preventDefault(); save.reset(); save.mutate({ recordMode: 'demo_visual_fields', strategyCode: 'demo_sita_standard', patternCode: 'demo_24_2', eyes: both ? [{ eye: 'right', resultCode: right }, { eye: 'left', resultCode: left }] : [{ eye: 'right', resultCode: right }], comment }) }
  ...
  return <section className="examination-draft-demo" aria-labelledby={`visual-fields-demo-${episodeId}`}>
    ...
    <form className="examination-draft-demo-form" onSubmit={submit}>
      ...
      <label><span>Eyes recorded</span><select value={both ? 'both' : 'right'} onChange={e => setBoth(e.target.value === 'both')}><option value="both">Both eyes</option><option value="right">Right eye</option></select></label>
      ...
      {both && <label><span>Left-eye result</span><select value={left} onChange={e => setLeft(e.target.value as typeof left)}>{results.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>}
